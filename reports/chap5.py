...
from osdagbridge.core.reports.report_charts import save_ur_summary_chart
import os

if TYPE_CHECKING:
    pass
def ch5_design_checks(checks_data, bridge, chart_paths=None, assets_dir=None) -> str:
    """Chapter 5 — Design Checks.
...
\hline"""
    ...
    # Governing UR per primary element family (for Overall Design Check chart)
    def _ur_num(cells):
    ...
    def _max_ur(*cell_sets):
        vals = [v for v in (_ur_num(c) for c in cell_sets) if v is not None]
        return max(vals) if vals else None

    _girder_ur = _max_ur(
        _dcr_row({1}), _dcr_row({2}), _dcr_row({5}),
        _dcr_row({13, 14}, fallback_unit="mm"),
        _dcr_row({11}, fallback_unit="MPa"),
        _dcr_row({8, 9}, fallback_unit="MPa"),
    )
    _deck_ur = _max_ur(
        _deck_row(KEY_DD_M_ULS_SAG, KEY_DD_MU_BOT, "kN-m/m"),
        _deck_row(KEY_DD_M_ULS_HOG, KEY_DD_MU_TOP, "kN-m/m"),
        _deck_row(KEY_DD_M_ULS_OH, KEY_DD_MU_OH, "kN-m/m", is_oh=True),
        _deck_row(KEY_DD_PUNCH_VED, KEY_DD_VRD_C_MPA, "MPa"),
        _deck_row(KEY_DD_SHEAR_VED, KEY_DD_SHEAR_VRDC, "kN/m"),
        _crack_cells,
    )
    _cb_ur = _max_ur(_cb_row("compression"), _cb_row("tension"), _cb_slender_row())
    if _ed_is_cb:
        _ed_ur = _max_ur(_cb_row("compression"), _cb_row("tension"))
    else:
        _ed_ur = _max_ur(_cb_row("compression"), _cb_row("tension"))
    if _ed_ur is None:
        _ed_ur = _cb_ur if _cb_ur is not None else 0.0
    if _girder_ur is None:
        _girder_ur = 0.0
    if _deck_ur is None:
        _deck_ur = 0.0
    if _cb_ur is None:
    ...
    ur_summary = {
        "Steel Plate Girders": _girder_ur,
        "Concrete Deck Slab": _deck_ur,
        "Cross Bracing": _cb_ur,
        "End Diaphragms": _ed_ur,
    }

    _ur_chart_tex = ""
    _ur_path = None
    if chart_paths and chart_paths.get("ur_summary"):
        _ur_path = chart_paths["ur_summary"]
    elif assets_dir:
        try:
            abs_png = os.path.join(assets_dir, "ur_summary.png")
            if save_ur_summary_chart(ur_summary, abs_png):
                _ur_path = "assets/ur_summary.png"
        except Exception:
            _ur_path = None
    if _ur_path:
        _ur_chart_tex = (
            "\n\\vspace{1.2em}\n"
            r"\subsection*{Utilization Ratio Visualisation}" + "\n"
            r"\noindent Governing demand/capacity ratios for the primary element "
            r"families are plotted below. Bars are green when UR $\leq 1.0$ (pass) "
            r"and red when UR $> 1.0$ (fail)." + "\n"
            + _fig_embed(
                _ur_path,
                r"Figure --- Utilization Ratio (Demand/Capacity) for Steel Plate "
                r"Girders, Concrete Deck Slab, Cross Bracing and End Diaphragms. "
                r"Red dashed line marks UR $= 1.0$ (pass/fail threshold).",
...
\vspace{0.4em}
\noindent
""" + styles.compact_table_begingroup() + r"""
\setlength\LTleft{0pt}
\setlength\LTright{\fill}
...
\end{longtable}
\noindent\textit{Note: IS 800 Cl.\ 3.8 --- Limit $= 250$ for compression members, $400$ for tension members. $K = 1.0$ for members with both ends pinned.}
...
\vspace{0.4em}
\noindent
""" + styles.local_table_spacing(styles.TABCOLSEP_COMPACT) + r"""
\setlength{\LTleft}{0pt}
\setlength{\LTright}{\fill}
...
""" + longtable_repeating_headers(
        ...
        r"\caption{\textbf{Overall Design Check Summary --- All Members}}",
        r"""\hline

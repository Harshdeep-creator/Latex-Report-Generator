...
% Packages
\usepackage[""" + styles.geometry_options() + r"""]{geometry}
\usepackage{graphicx}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{booktabs}
...
% Table layout — from styles.py
""" + styles.table_layout_preamble() + r"""

\definecolor{osdagGreen}{HTML}{""" + styles.COLOR_OSDAG_GREEN + r"""}
...
\fancypagestyle{main}{
  ...
  \renewcommand{\headrule}{\color{osdagGreen}\hrule width\headwidth height """ + styles.HEADRULE_HEIGHT + r""" \vspace{2pt}}
  \renewcommand{\footrule}{%
    \ifbool{hasSDonPage}{%
      \vspace{""" + styles.SD_NOTE_VSPACE_BEFORE + r"""}%
      \hbox to \headwidth{\textcolor{black}{\footnotesize\textit{* Software default value}}\hfil}%
      \vspace{""" + styles.SD_NOTE_VSPACE_AFTER + r"""}%
    }{%
      \vspace{""" + styles.FOOTER_CLEARANCE + r"""}%
    }%
            \color{osdagGreen}\hrule width\headwidth height """ + styles.FOOTRULE_HEIGHT + r""" \vspace{""" + styles.FOOTRULE_AFTER + r"""}%
  }
}
...
\fancypagestyle{firstpage}{
  ...
  \fancyfoot[L]{Osdag $|$ FOSSEE $|$ Indian Institute of Technology Bombay}
  \fancyfoot[R]{Page \thepage\ of \pageref{LastPage}}
  \renewcommand{\footrule}{\vspace{""" + styles.FOOTER_CLEARANCE + r"""}\color{osdagGreen}\hrule width\headwidth height """ + styles.FOOTRULE_HEIGHT + r""" \vspace{""" + styles.FOOTRULE_AFTER + r"""}}
}
\pagestyle{main}
\setstretch{1.15}
...
def title_page(m, osdag_logo, org_logo):
    ...
    if osdag_logo:
        logo_block = (
            r"\includegraphics[width=" + styles.TITLE_LOGO_WIDTH
            + r"]{" + osdag_logo.replace("\\", "/") + r"}\\[0.85cm]"
        )
    else:
        logo_block = r"{\Huge \textbf{Osdag}}\\[0.4cm]"

    org_block = ""
    if org_logo:
        org_block = (
            r"\includegraphics[height=" + styles.TITLE_ORG_LOGO_HEIGHT
            + r",keepaspectratio]{" + org_logo.replace("\\", "/") + r"}\\[0.7cm]"
        )
...
\vspace*{1.1cm}
""" + logo_block + r"""
{\color{osdagGreen}\rule{0.72\textwidth}{1.2pt}}\\[0.55cm]
{\LARGE \textbf{OsdagBridge}}\\[0.25cm]
{\large Open Source Software for Steel Girder Bridge Design}\\[0.55cm]
{\Large \textbf{Design Report}}\\[0.7cm]
""" + org_block + r"""
...
def generate_report(payload, request):
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            for cand in (
                os.path.join(_here, 'assets', 'osdag_logo.png'),
                os.path.join(_here, '..', 'data', 'ResourceFiles', 'vectors', 'Osdag Logo.png'),
            ):
                if os.path.exists(cand):
                    osdag_logo_src = cand
                    break
            iit_logo_src   = os.path.join(_here, 'assets', 'IIT Bombay Logo.png')
            ...
            if 'design_checks' in secs:
                doc_parts.append(ch5_design_checks(payload.design_checks, bridge, assets_dir=tmp_assets))
            if 'drawings' in secs and payload.options.include_figures:
                doc_parts.append(ch6_drawings(fig_paths))

            doc_parts.append(ch7_quantities(payload.inputs, assets_dir=tmp_assets))
            ...
            # Guarantee repeating headers on EVERY longtable (Ch2–Ch7), including
            # legacy raw tables that pre-date the shared helper API.
            from osdagbridge.core.reports.report_utils import retrofit_longtables

            full_tex = retrofit_longtables(full_tex)

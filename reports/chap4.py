...
def ch4_analysis(asum, fig_paths, bridge: "ReportDataBridge", span_m: float):
    ...
    _ = styles.FIGURE_WIDTH_DEFAULT  # envelope figures use a narrower override below
...
\vspace{1em}
""" + styles.analysis_table_begingroup() + r"""
\begin{longtable}{|
...
""" + (
        ("\n\\hline\n").join(
            _tex(lc) + r" & " + _fmt(rxn_summary[lc].get("left_kN"))
            + r" & " + _fmt(rxn_summary[lc].get("right_kN")) + r" \\[6pt]"
